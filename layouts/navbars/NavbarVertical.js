...
// import routes file
import { DashboardMenu } from '../../routes/Routes'; // Adjust path as needed
import { useAuth } from '../../hooks/AuthOy'; // Import useAuth hook

const NavbarVertical = (props) => {
    const location = usePathname();
    const { user, isAuthenticated, loading } = useAuth(); // Access user, isAuthenticated, loading from AuthContext
        ...
        return (
            <li className="nav-item">
                <Link
                    ...
                    data-bs-target="#navDashboard" // This ID might need to be dynamic if multiple accordions exist
    ...
    const generateLink = (item) => {
        // Check if the item has roles and if the current user's role matches one of them
        const hasAccess = !item.roles || (isAuthenticated && user && item.roles.includes(user.role));

        if (!hasAccess) {
            return null; // Don't render link if user doesn't have access
        }
        ...
        return (
            <Link
                ...
                {item.icon ? <i className={`nav-icon fe fe-${item.icon} me-2`}></i> : ''} {/* Render icon here */}
                {item.name}
                {''}
                {item.badge ? (
                    <Badge
    ...
    // Don't render anything if auth state is still loading
    if (loading) {
        return null;
    }
            ...
            <SimpleBar style={{ maxHeight: '100vh' }}>
                ...
                <Accordion defaultActiveKey="0" as="ul" className="navbar-nav flex-column">
                    {DashboardMenu.map(function (menu, index) {
                        // Check access for top-level menu items
                        const hasAccess = !menu.roles || (isAuthenticated && user && menu.roles.includes(user.role));

                        if (!hasAccess) {
                            return null; // Skip rendering this menu item if user doesn't have access
                        }
                                        ...
                                        <Accordion.Collapse eventKey={index} as="li" bsPrefix="nav-item">
                                            <ListGroup as="ul" bsPrefix="" className="nav flex-column">
                                                {menu.children.map(function (menuLevel1Item, menuLevel1Index) {
                                                    // Check access for first-level child items
                                                    const hasLevel1Access = !menuLevel1Item.roles || (isAuthenticated && user && menuLevel1Item.roles.includes(user.role));
                                                    if (!hasLevel1Access) {
                                                        return null; // Skip rendering this child item
                                                    }
                                                                    ...
                                                                    <Accordion.Collapse eventKey={0} bsPrefix="nav-item">
                                                                        <ListGroup as="ul" bsPrefix="" className="nav flex-column">
                                                                            ...
                                                                            {menuLevel1Item.children.map(function (menuLevel2Item, menuLevel2Index) {
                                                                                // Check access for second-level child items
                                                                                const hasLevel2Access = !menuLevel2Item.roles || (isAuthenticated && user && menuLevel2Item.roles.includes(user.role));
                                                                                if (!hasLevel2Access) {
                                                                                    return null; // Skip rendering this child item
                                                                                }
                                                                                                ...
                                                                                                <Accordion.Collapse eventKey={0} bsPrefix="nav-item">
                                                                                                    <ListGroup as="ul" bsPrefix="" className="nav flex-column">
                                                                                                        ...
                                                                                                        {menuLevel2Item.children.map(function (menuLevel3Item, menuLevel3Index) {
                                                                                                            // Check access for third-level child items
                                                                                                            const hasLevel3Access = !menuLevel3Item.roles || (isAuthenticated && user && menuLevel3Item.roles.includes(user.role));
                                                                                                            if (!hasLevel3Access) {
                                                                                                                return null; // Skip rendering this child item
                                                                                                            }
                                                                                                            return (
